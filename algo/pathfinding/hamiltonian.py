...
def find_nearest_neighbor_path(start, obstacles):
    ...
    while obstacles:
        nearest_neighbor = min(obstacles, key=lambda obstacle: abs(obstacle.x_g - current_pos[0]) + abs(obstacle.y_g - current_pos[1]))
    ...
    for y in range(grid_size - 1, -1, -1):
        for x in range(grid_size):
            ...
            if (0 <= x <= 2) and (0 <= y <= 2):
                ...
                direction = next((obstacle.facing for obstacle in obstacles if (obstacle.x_g, obstacle.y_g) == position), None)
